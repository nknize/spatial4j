...
public abstract class AbstractCRSDelegate implements CRSDelegate {
  ...
  @Override
  public Rectangle getBBox(Point source, Point sink, SpatialContext ctx) {
    return new RectangleImpl(Math.min(source.getX(), sink.getX()), Math.max(source.getX(), sink.getX()),
            Math.min(source.getY(), sink.getY()), Math.max(source.getY(), sink.getY()), ctx);
  }
}
