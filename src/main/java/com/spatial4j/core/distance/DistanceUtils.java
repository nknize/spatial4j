...
public class DistanceUtils {
  ...
  /**
   * If latitude is normalized from -90 < lat || lat > 90, to -90 <= lat <= 90
   * then longitude needs to be normalized to its antipodal point, <code>normPoint</code>
   * should be used in place of <code>normLonDEG</code> and <code>normLatDEG</code> to
   * keep the lat/lon pairs consistent.
   *
   * @param pt
   * @return
   */
  public static Point normPoint(Point pt) {
    ...
    if (lat>90.0 || lat<-90.0) {
      ...
      double off = ((lat % 360.0 + 360.0) % lat) + 90.0; // correct mod operator for negative numbers
